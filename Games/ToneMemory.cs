...
public class ToneMemory : Stimuli
{
    ...
    // Update
    // Number of orbs used
    private static int Number_Of_Orbs = 5;
    // Lists for iterating info
    private GameObject[] Orbs;

    // The names of each orb gameobject in the scene
    private string[] Orb_Object_Names = {
        "Target_One",   // First orb
        "Target_Two",   // Second orb
        "Target_Three", // Third orb
        "Target_Four",  // Fourth orb
        "Target_Five"   // Fifth orb
    };
    // The perspective positions of each orb
    private Vector3[] Orb_Pos = {
        new Vector3(-0.2f, 1.45f, -0.162f),     // Target_One Position
        new Vector3(-0.23f, 1.73f, 3.086f),     // Target_Two Position
        new Vector3(-0.243f, 2.0f, 6.334f),     // Target_Three Position
        new Vector3(-0.263f, 2.277f, 9.582f),   // Target_Four Position
        new Vector3(-0.29f, 2.55f, 12.83f)      // Target_Five Position
    };

    // Triggers based on [target][pos]
    // Where target = 1 if it is a target 0 if standard
    int[] Update_triggers = new int[]{
            { 75, 76, 77, 78, 79}
        };
    int[] EndRecall_triggers = new int[]{
            {80, 85, 87, 88, 89}
        };
    int[] PlayRandomSound_triggers = new int[]{
            {69, 71, 72, 73, 74}
        };
    ...
    // Update
    // Initialize the attributes of ToneMemory
    public ToneMemory()
    {
        // Initialize the Orb objects
        Orbs = new GameObject[Number_Of_Orbs];
        for(int i = 0; i < Number_Of_Orbs; ++i)
        {
            Orbs[i] = GameObject.Find(Orb_Object_Names[i]);
            Orbs[i].transform.localPosition = Hide;

            //Now let's turn off shadows for our objects
            Orbs[i].transform.GetComponent<Renderer>().shadowCastingMode = UnityEngine.Rendering.ShadowCastingMode.Off;
        }
        ...
        for (var i_dist = 0; i_dist < 5; i_dist++)
        {
            for (var i_col = 0; i_col < 50; i_col++)
            {
                if (i_dist > 0)
                {
                    ...
                    trial_dist[1, (i_col + (50 * i_dist))] = i_dist;
                }
                else
                {
                    trial_dist[0, (i_col + (50 * i_dist))] = 0;
                    trial_dist[1, (i_col + (50 * i_dist))] = i_dist;
                }
            }
        }
...
# endif
    ...
    IEnumerator CycleThroughDepths()
    {
        ...
        for(int i = 0; i < Number_Of_Orbs; ++i)
        {
            ...
            User_Info_Text.text = Orb_Object_Names[i];
            User_Info.gameObject.transform.localPosition = User_Info_Pos;
            Orbs[i].gameObject.transform.localPosition = Orb_Pos[i];
    ...
    void EndRecall()
    {
        var pos = trial_dist[1, trialNum];

        Orbs[pos].gameObject.transform.localPosition = Hide;
        trigger = EndRecall_triggers[pos];
    ...
    void PlayRandomSound()
    {
        ...
        if (_audioManager != null)
        {
            ...
            Orbs[pos].gameObject.transform.localPosition = Orb_Pos[pos];
            trigger = Update_triggers[pos];
        }
